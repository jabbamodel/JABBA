...
png(file = paste0(input.dir,"/Catches_",assessment,".png"), width = 7, height = 5, 
    res = 200, units = "in")
par(Par)
plot(catch[,1],catch[,1],ylim=c(0,max(catch[,2:ncol(catch)],na.rm=TRUE)),ylab="Catch (tons)",xlab="Year",type="n")
...
if(sigma.proc==TRUE){
  ...
  legend("topright",paste(names(cpue)[qs+1]),lwd=2,col=rainbow(ncol(mCPUE)),bty="n")
  polygon(x = c(cpue.yrs,rev(cpue.yrs)), y = c(lower,rev(upper)), col = "gray", border = "gray90")
  
  for(i in 1:n.indices)
  {
    shift = runif(1,-0.1,0.1)
    plotCI(cpue.yrs+shift,mCPUE[,i]/q.adj[i],ui=exp(log(mCPUE[,i]/q.adj[i])+1.96*sqrt(mSE2[,i])),li=exp(log(mCPUE[,i]/q.adj[i])-1.96*sqrt(mSE2[,i])),add=TRUE,col= rainbow(ncol(mCPUE))[i],pt.bg = rainbow(ncol(mCPUE))[i],pch=21,gap=0)
    lines(cpue.yrs+shift,mCPUE[,i]/q.adj[i], col = rainbow(ncol(mCPUE))[i],lwd=2)
    points(cpue.yrs+shift,mCPUE[,i]/q.adj[i], bg = rainbow(ncol(mCPUE))[i],pch=21)
  }
...
if(shape==FALSE){
  if(Model == 1 | Model == 4){m=2} else {m = 1.001}}else{m=shape}
...
SP = ifelse(P>Plim,r.pr[1]/(1-m)*P*(1-P^(m-1)),r.pr[1]/(1-m)*P*(1-P^(m-1))*4*P)
...
plot(years,(TC/1000),type="l",ylim=c(0,max((TC)*0.001)),lty=1,lwd=1.3,xlab="Year",ylab="Catch ('000 tons)",main="")
...
png(file = paste0(output.dir,"/Fits_",assessment,"_",Scenario,".png"), width = 7, height = ifelse(n.indices==1,5,2.5)*round(series/2+0.01,0), 
    res = 200, units = "in")
par(Par)
...
# JABBA-residual plot ## LOG RESIDS 
Par = list(
  mfrow = c(1, 1),
...
plot(
  ...
  type = "n",
  ylim =
    ifelse(rep(max(Resids, na.rm = T), 2) > 0.9,
           range(1.2 * Resids, na.rm = T),
           range(c(-1.3, 1.2))),
  xlim = range(cpue.yrs),
  ylab = "log residuals",
  xlab = "Year"
...
for (i in 1:length(indices)){
  for (t in 1:n.years) {
    # lines(rep((Yr+positions[i])[t],2),c(0,Resids[i,t]),col=rainbow(series)[i])}
    lines(rep((Yr + positions[i])[t], 2), c(0, Resids[i, t]),
...
legend(
  'bottomright',
  bty = "n",
  pch = c(rep(20, series), -1),
  cex = 1.1,
  legend = c(paste(wink.colors[which(wink.colors[, 'idx'] %in% indices), 'idx']), "Loess"),
  col = c(as.character(wink.colors[which(wink.colors[, 'idx'] %in% indices), 'cols']),'black'),
  bg = c(paste(wink.colors[which(wink.colors[, 'idx'] %in% indices), 'idx']), "Loess"),
  lwd = c(rep(-1, series), 2)
)
...
for (i in 1:series) {
  StResid = rbind(
    StResid,
    log(CPUE[, i] / apply(posteriors$CPUE[, , i], 2, quantile, c(0.5))) /
      apply(posteriors$TOE[, , i], 2, quantile, c(0.5)) + 0.5 *
      apply(posteriors$TOE[, , i], 2, quantile, c(0.5))
  )
}
...
for (i in 1:length(indices)){
  for (t in 1:n.years) {
    lines(rep((Yr + positions[i])[t], 2), c(0, StResid[i, t]), 
          col = as.character(wink.colors[wink.colors$idx == indices[i], 'cols']))
  }
  points(
    Yr + positions[i],
...
plot(Bit,SP,type = "n",ylim=c(0,max(c(max(Catch,na.rm=T),max(MSY*1.1)))),ylab="Surplus Production (t)",xlab="Biomass (t)")
...
plot(Bit,SP,type = "n",ylim=c(0,max(c(max(Catch,na.rm=T)*1.05,max(MSY*1.1)))),xlim=c(0,max(Bit,B)),ylab="Surplus Production (t)",xlab="Biomass (t)",xaxs="i",yaxs="i")
...
if(Projection ==TRUE){
  ...
  legend("topleft",paste(TACs,"(t)"),col=rev(rainbow(nTAC)),lwd=2,cex=0.8)   
